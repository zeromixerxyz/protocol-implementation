...
let web3, ZeroMixer, circuit, proving_key, groth16, erc20, senderAccount, netId
...
async function deposit({ currency, amount }) {
  ...
  const noteString = `ZeroMixer-${currency}-${amount}-${netId}-${note}`
  console.log(`Your note: ${noteString}`)
  if (currency === 'eth') {
    await printETHBalance({ address: ZeroMixer._address, name: 'ZeroMixer' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
    const value = isLocalRPC ? ETH_AMOUNT : fromDecimals({ amount, decimals: 18 })
    console.log('Submitting deposit transaction')
    await ZeroMixer.methods.deposit(toHex(deposit.commitment)).send({ value, from: senderAccount, gas: 2e6 })
    await printETHBalance({ address: ZeroMixer._address, name: 'ZeroMixer' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
  } else { // a token
    await printERC20Balance({ address: ZeroMixer._address, name: 'ZeroMixer' })
    ...
    const allowance = await erc20.methods.allowance(senderAccount, ZeroMixer._address).call({ from: senderAccount })
    console.log('Current allowance is', fromWei(allowance))
    if (toBN(allowance).lt(toBN(tokenAmount))) {
      console.log('Approving tokens for deposit')
      await erc20.methods.approve(ZeroMixer._address, tokenAmount).send({ from: senderAccount, gas: 1e6 })
    }

    console.log('Submitting deposit transaction')
    await ZeroMixer.methods.deposit(toHex(deposit.commitment)).send({ from: senderAccount, gas: 2e6 })
    await printERC20Balance({ address: ZeroMixer._address, name: 'ZeroMixer' })
    await printERC20Balance({ address: senderAccount, name: 'Sender account' })
  }
 ...
 * Download deposit events from the ZeroMixer, reconstructs merkle tree, finds our deposit leaf
 * in it and generates merkle proof
 * @param deposit Deposit object
 */
async function generateMerkleProof(deposit) {
  // Get all deposit events from smart contract and assemble merkle tree from them
  console.log('Getting current state from ZeroMixer contract')
  const events = await ZeroMixer.getPastEvents('Deposit', { fromBlock: 0, toBlock: 'latest' })
  const leaves = events
  ...
  const isValidRoot = await ZeroMixer.methods.isKnownRoot(toHex(root)).call()
  const isSpent = await ZeroMixer.methods.isSpent(toHex(deposit.nullifierHash)).call()
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    try {
      const relay = await axios.post(relayerURL + '/relay', { contract: ZeroMixer._address, proof, args })
      if (netId === 1 || netId === 42) {
    ...
    await ZeroMixer.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
...
/**
 * Parses ZeroMixer.xyz note
 * @param noteString the note
 */
function parseNote(noteString) {
  const noteRegex = /ZeroMixer-(?<currency>\w+)-(?<amount>[\d.]+)-(?<netId>\d+)-0x(?<note>[0-9a-fA-F]{124})/g
...
async function loadDepositData({ deposit }) {
  try {
    const eventWhenHappened = await ZeroMixer.getPastEvents('Deposit', {
      filter: {
    ...
    const isSpent = await ZeroMixer.methods.isSpent(deposit.nullifierHex).call()
...
async function loadWithdrawalData({ amount, currency, deposit }) {
  try {
    const events = await ZeroMixer.getPastEvents('Withdrawal', {
      fromBlock: 0,
      toBlock: 'latest',
    })
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  let contractJson, erc20ContractJson, erc20ZeroMixerJson, ZeroMixerAddress, tokenAddress
  // TODO do we need this? should it work in browser really?
  if (inBrowser) {
    // Initialize using injected web3 (Metamask)
    // To assemble web version run `npm run browserify`
    web3 = new Web3(window.web3.currentProvider, null, { transactionConfirmationBlocks: 1 })
    contractJson = await (await fetch('build/contracts/ETHZeroMixer.json')).json()
    ...
    contractJson = require(__dirname + '/../build/contracts/ETHZeroMixer.json')
    ...
    erc20ZeroMixerJson = require(__dirname + '/../build/contracts/ERC20ZeroMixer.json')
  }
  ...
  if (isLocalRPC) {
    ZeroMixerAddress = currency === 'eth' ? contractJson.networks[netId].address : erc20ZeroMixerJson.networks[netId].address
    tokenAddress = currency !== 'eth' ? erc20ContractJson.networks[netId].address : null
    senderAccount = (await web3.eth.getAccounts())[0]
  } else {
    try {
      ZeroMixerAddress = config.deployments[`netId${netId}`][currency].instanceAddress[amount]
      if (!ZeroMixerAddress) {
        throw new Error()
      }
      tokenAddress = config.deployments[`netId${netId}`][currency].tokenAddress
    } catch (e) {
      console.error('There is no such ZeroMixer instance, check the currency and amount you provide')
      process.exit(1)
    }
  }
  ZeroMixer = new web3.eth.Contract(contractJson.abi, ZeroMixerAddress)
...
async function main() {
  if (inBrowser) {
    ...
    program
      ...
      .description('Submit a deposit of specified currency and amount from default eth account and return the resulting note. The currency is one of (ETH|DAI|cDAI|USDC|cUSDC|USDT). The amount depends on currency, see config.js file or visit https://ZeroMixer.xyz.')
