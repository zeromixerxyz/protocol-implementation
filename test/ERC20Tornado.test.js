...
const ZeroMixer = artifacts.require('./ERC20ZeroMixer.sol')
...
contract('ERC20ZeroMixer', (accounts) => {
  let ZeroMixer
  let token
  let usdtToken
  let badRecipient
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const tokenFromContract = await ZeroMixer.token()
  ...
  describe('#deposit', () => {
    it('should work', async () => {
      ...
      await token.approve(ZeroMixer.address, tokenDenomination)

      let { logs } = await ZeroMixer.deposit(commitment, { from: sender })
    ...
    it('should not allow to send ether on deposit', async () => {
      ...
      let error = await ZeroMixer.deposit(commitment, { from: sender, value: 1e6 }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      await token.approve(ZeroMixer.address, tokenDenomination, { from: user })
      // Uncomment to measure gas usage
      // let gas = await ZeroMixer.deposit.estimateGas(toBN(deposit.commitment.toString()), { from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await ZeroMixer.deposit(toFixedHex(deposit.commitment), { from: user, gasPrice: '0' })
      ...
      const balanceZeroMixerBefore = await token.balanceOf(ZeroMixer.address)
      ...
      const { logs } = await ZeroMixer.withdraw(proof, ...args, { value: refund, from: relayer, gasPrice: '0' })

      const balanceZeroMixerAfter = await token.balanceOf(ZeroMixer.address)
      ...
      balanceZeroMixerAfter.should.be.eq.BN(toBN(balanceZeroMixerBefore).sub(toBN(tokenDenomination)))
    ...
    it('should reject with wrong refund value', async () => {
      ...
      let { reason } = await ZeroMixer.withdraw(proof, ...args, { value: 1, from: relayer, gasPrice: '0' })
        .should.be.rejected
      reason.should.be.equal('Incorrect refund amount received by the contract')
      ;({ reason } = await ZeroMixer.withdraw(proof, ...args, {
    ...
    it.skip('should work with REAL USDT', async () => {
      ...
      await usdtToken.approve(ZeroMixer.address, tokenDenomination, { from: user })
      console.log('approve done')
      const allowanceUser = await usdtToken.allowance(user, ZeroMixer.address)
      ...
      const balanceZeroMixerBefore = await usdtToken.balanceOf(ZeroMixer.address)
      ...
      const ethBalanceReceiverBefore = await web3.eth.getBalance(toFixedHex(recipient, 20))
      let isSpent = await ZeroMixer.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(false)
      ...
      const balanceZeroMixerAfter = await usdtToken.balanceOf(ZeroMixer.address)
      ...
      logs[0].args.fee.should.be.eq.BN(feeBN)
      isSpent = await ZeroMixer.isSpent(input.nullifierHash.toString(16).padStart(66, '0x00000'))
      isSpent.should.be.equal(true)
    })
