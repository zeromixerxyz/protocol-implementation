...
const ZeroMixer = artifacts.require('./ETHZeroMixer.sol')
...
contract('ETHZeroMixer', (accounts) => {
  let ZeroMixer
  ...
  before(async () => {
    ...
    ZeroMixer = await ZeroMixer.deployed()
  ...
  describe('#constructor', () => {
    it('should initialize', async () => {
      const etherDenomination = await ZeroMixer.denomination()
  ...
  describe('#deposit', () => {
    it('should emit event', async () => {
      ...
      let { logs } = await ZeroMixer.deposit(commitment, { value, from: sender })
      ...
      ;({ logs } = await ZeroMixer.deposit(commitment, { value, from: accounts[2] }))
    ...
    it('should throw if there is a such commitment', async () => {
      ...
      await ZeroMixer.deposit(commitment, { value, from: sender }).should.be.fulfilled
      const error = await ZeroMixer.deposit(commitment, { value, from: sender }).should.be.rejected
  ...
  describe('#withdraw', () => {
    it('should work', async () => {
      ...
      // let gas = await ZeroMixer.deposit.estimateGas(toBN(deposit.commitment.toString()), { value, from: user, gasPrice: '0' })
      // console.log('deposit gas:', gas)
      await ZeroMixer.deposit(toFixedHex(deposit.commitment), { value, from: user, gasPrice: '0' })
      ...
      const balanceZeroMixerBefore = await web3.eth.getBalance(ZeroMixer.address)
      ...
      let isSpent = await ZeroMixer.isSpent(toFixedHex(input.nullifierHash))
      isSpent.should.be.equal(false)

      // Uncomment to measure gas usage
      // gas = await ZeroMixer.withdraw.estimateGas(proof, publicSignals, { from: relayer, gasPrice: '0' })
      ...
      const { logs } = await ZeroMixer.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })

      const balanceZeroMixerAfter = await web3.eth.getBalance(ZeroMixer.address)
      ...
      balanceZeroMixerAfter.should.be.eq.BN(toBN(balanceZeroMixerBefore).sub(toBN(value)))
      ...
      isSpent = await ZeroMixer.isSpent(toFixedHex(input.nullifierHash))
    ...
    it('should prevent double spend', async () => {
      const deposit = generateDeposit()
      tree.insert(deposit.commitment)
      await ZeroMixer.deposit(toFixedHex(deposit.commitment), { value, from: sender })
      ...
      await ZeroMixer.withdraw(proof, ...args, { from: relayer }).should.be.fulfilled
      const error = await ZeroMixer.withdraw(proof, ...args, { from: relayer }).should.be.rejected
    ...
    it('should reject with tampered public inputs', async () => {
      ...
      let error = await ZeroMixer.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      ...
      error = await ZeroMixer.withdraw(proof, ...incorrectArgs, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Invalid withdraw proof')
      ...
      await ZeroMixer.withdraw(proof, ...args, { from: relayer }).should.be.rejected

      // should work with original values
      await ZeroMixer.withdraw(originalProof, ...args, { from: relayer }).should.be.fulfilled
    })
  ...
  describe('#isSpent', () => {
    it('should work', async () => {
      ...
      await ZeroMixer.deposit(toFixedHex(deposit1.commitment), { value, gasPrice: '0' })
      await ZeroMixer.deposit(toFixedHex(deposit2.commitment), { value, gasPrice: '0' })
      ...
      await ZeroMixer.withdraw(proof, ...args, { from: relayer, gasPrice: '0' })
      ...
      const spentArray = await ZeroMixer.isSpentArray([nullifierHash1, nullifierHash2])
